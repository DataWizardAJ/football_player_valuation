...
@st.cache_data(ttl=604800)  # refresh weekly, matching transfermarkt-datasets update frequency
def load_data():
    raw_data   = pd.read_csv(os.path.join(BASE_DIR, 'paid_player_transfers_with_stats.csv'))
    stats_live = run_pipeline()  # pulls and engineers live data from GitHub
    
    # Filter to current season for the predictor
    current_season = stats_live['season'].max()
    stats_2026     = stats_live[stats_live['season'] == current_season]
    
    return raw_data, stats_2026

pipeline, rf_no_mv   = load_models()
raw_data, stats_2026 = load_data()
...
with tab2:
    ...
    all_2026_players = sorted(stats_2026['player_name'].dropna().unique().tolist())
    all_clubs        = sorted(raw_data['to_club_name'].dropna().unique().tolist())

    player_name_pred  = st.selectbox('Select a player:', [''] + all_2026_players, key='tab2_player')
